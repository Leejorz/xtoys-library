...
function loadVideo() {
    ...
    // xToys does not provide native embedded playback for these source families.
    // Keep the funscript loaded but idle until the user explicitly presses
    // the existing "Start manually" button.
    if (video && isExternalVideoSite(video.site)) {
        console.log("External video source detected: " + video.site + " / " + video.id);
        statusText("External video source: start the video separately, then press Start manually");
        displayText(
            "External video source detected.\nOpen the video separately, then press Start manually when ready.",
            0,
            "none",
            true
        );
